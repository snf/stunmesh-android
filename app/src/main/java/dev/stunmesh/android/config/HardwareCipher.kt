...
/** Non-exportable hardware wrapping key. The WG private key is deliberately
 * usable in this process and in the authorized OS backup snapshot. */
internal class HardwareCipher(private val context:Context) {
    private fun key(create:Boolean):SecretKey {
        ...
        if(key==null){
            ...
            key=if(context.packageManager.hasSystemFeature(PackageManager.FEATURE_STRONGBOX_KEYSTORE)){
        ...
        val secure=if(Build.VERSION.SDK_INT>=31)info.securityLevel in setOf(KeyProperties.SECURITY_LEVEL_STRONGBOX,KeyProperties.SECURITY_LEVEL_TRUSTED_ENVIRONMENT) else {
    ...
    fun encrypt(plain:ByteArray):ByteArray {
        ...
        val c=Cipher.getInstance("AES/GCM/NoPadding");c.init(Cipher.ENCRYPT_MODE,key(true));c.updateAAD(HEADER)
    ...
    fun decrypt(blob:ByteArray):ByteArray {
        ...
        val c=Cipher.getInstance("AES/GCM/NoPadding");c.init(Cipher.DECRYPT_MODE,key(false),GCMParameterSpec(128,blob,HEADER.size,12));c.updateAAD(HEADER)
        return c.doFinal(blob,HEADER.size+12,blob.size-HEADER.size-12)
    }
    private companion object {const val ALIAS="stunmesh_config_v1";val HEADER=byteArrayOf(0x53,0x4e,0x46,1)}
}
