 ...
 * excerpts in errors. Anchors, aliases, nulls and duplicate keys are rejected before producing a
...
object StrictDocument {
    ...
    fun parse(text: String): JSONObject =
        try {
            ...
            val node =
                Yaml(SafeConstructor(options)).compose(StringReader(text))
                    ?: throw IllegalArgumentException()
            convert(node, 0) as? JSONObject ?: throw IllegalArgumentException()
        } catch (_: Exception) {
            throw IllegalArgumentException("Invalid or oversized configuration")
        }
...
internal fun JSONObject.text(name: String, fallback: String = ""): String =
    if (!has(name)) fallback
    else get(name) as? String ?: throw IllegalArgumentException("Invalid text field")

internal fun JSONObject.number(name: String, fallback: Int): Int =
    if (!has(name)) fallback
    else get(name) as? Int ?: throw IllegalArgumentException("Invalid number field")

internal fun JSONObject.obj(name: String): JSONObject =
    if (!has(name)) JSONObject()
    else get(name) as? JSONObject ?: throw IllegalArgumentException("Invalid object field")

internal fun JSONObject.array(name: String): JSONArray =
    if (!has(name)) JSONArray()
    else get(name) as? JSONArray ?: throw IllegalArgumentException("Invalid list field")

internal fun JSONObject.strings(name: String): List<String> =
    array(name).let { array ->
        (0 until array.length()).map {
            array.get(it) as? String ?: throw IllegalArgumentException("Invalid list value")
        }
    }
