...
/**
 * Private in-process model. It is serialized only to the core, protected local storage or the OS
 * backup agent. UI/sharing uses PublicTunnel, never this model.
 */
data class TunnelConfig(
    ...
    companion object {
        ...
        fun fromJsonObject(o: JSONObject): TunnelConfig {
            ...
            return TunnelConfig(
                id = o.text("id").ifEmpty { UUID.randomUUID().toString() },
                name = o.text("name", "Server"),
                iface = InterfaceConfig.fromJson(o.obj("interface")),
                peers =
                    o.array("peers").let { a ->
                        (0 until a.length()).map { PeerConfig.fromJson(a.getJSONObject(it)) }
                    },
                plugins =
                    o.array("plugins").let { a ->
                        (0 until a.length()).map { PluginDefinition.fromJson(a.getJSONObject(it)) }
                    },
                stunServers = stun.strings("addresses"),
                refreshIntervalSeconds = o.number("refresh_interval_seconds", 180),
                logLevel = log.text("level", "info"),
                proposalId = o.text("proposal_id"),
            )
        }
    }
}
...
data class InterfaceConfig(
    ...
    companion object {
        fun fromJson(o: JSONObject): InterfaceConfig {
            o.fields("private_key", "addresses", "dns_servers", "listen_port", "mtu", "protocol")
            return InterfaceConfig(
                o.text("private_key"),
                o.strings("addresses"),
                o.strings("dns_servers"),
                o.number("listen_port", 0),
                o.number("mtu", 1420),
                o.text("protocol", "ipv4"),
            )
        }
    }
}
...
data class PeerConfig(
    ...
    companion object {
        fun fromJson(o: JSONObject): PeerConfig {
            o.fields(
                "name",
                "description",
                "public_key",
            ...
            return PeerConfig(
                ...
                o.text("description"),
                o.text("public_key"),
                o.text("preshared_key"),
                o.strings("allowed_ips"),
                o.text("endpoint"),
                o.text("plugin"),
                o.text("protocol", "ipv4"),
                o.number("persistent_keepalive", 25),
            )
        }
    }
}
...
data class PluginDefinition(
    val instance: String = "dht",
    val type: String = "builtin",
    val name: String = "opendht",
    ...
    companion object {
        fun fromJson(o: JSONObject): PluginDefinition {
            o.fields("instance", "type", "name", "config")
            val c = o.obj("config")
            c.fields("endpoint", "endpoints", "timeout", "dedup")
            val values =
                c.keys().asSequence().associateWith { key ->
                    when (key) {
                        "endpoints" -> c.strings(key)
                        "dedup" -> c.get(key).also { require(it == false) }
                        else -> c.text(key)
                    }
                }
...
data class PublicTunnel(
    val id: String,
    val name: String,
    val publicKey: String,
    val serverKeys: List<String>,
