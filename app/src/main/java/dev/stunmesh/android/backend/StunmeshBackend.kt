...
fun interface TunProvider { fun openTun(mtu:Int):Int }
fun interface SocketProtector { fun protect(fd:Int):Boolean }
interface EventListener {
    fun onStateChanged(state:BackendState)
    fun onLog(level:String,message:String)
    fun onEvent(event:BackendEvent)
}
enum class BackendState { DOWN, STARTING, UP, STOPPING }
